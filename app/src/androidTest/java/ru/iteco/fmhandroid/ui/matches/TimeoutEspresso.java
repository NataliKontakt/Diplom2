...
public class TimeoutEspresso {
    //Добавил тебе комменты для пояснений


    // Эта переменная отвечает за время обновления запроса на появление селектора в милисекундах.
    private static final int SLEEP_IN_A_LOOP_TIME = 50;

    // Эта переменная отвечает за время, в течении которого будет опрашиваться система на появление селектора в милисекундах.
    ...
    // Вместо обычного "OnView" используешь этот "onViewWithTimeout" и дальше те же селекторы как и просто при использовании Espreso.
    ...
    // Переиспользование метода "onViewWithTimeout" с дефолтным таймаутом. - Из перменной объявленной выше.
    ...
    public static class TimedViewInteraction {
        ...
        // ViewInteraction - это класс для вьюшек (при использовании OnView) от espresso. TimedViewInteraction - просто приравнивает одно к другому.
